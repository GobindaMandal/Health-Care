<x-app-layout>
  <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
      <div class="container mx-auto px-6 py-2">
        ...
        <form action="{{ route('filter.boardsApplication') }}" method="GET">
            <div class="row mb-3">
              <div class="col-md-3">
              ...
              <div class="col-md-3">
                ...
                <select class="form-control form-control-sm text-sm" name="search_date" id="search_date">
                  ...
                  @foreach($application_form as $application)
                    @if (!isset($meeting_dates[$application->meeting_date]) && !empty($application->meeting_date))
                      <option value="{{$application->meeting_date}}">{{$application->meeting_date}}</option>
                      <?php $meeting_dates[$application->meeting_date] = true; ?>
                    @endif
                  @endforeach
                </select>
              </div>
            ...
            <tbody>
              @can('Boards application access')
                ...
                <tr class="hover:bg-grey-lighter">
                  ...
                  @if(App\Models\Back\Daughter_Marriage::where('application_id', $report->id)->first())
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Daughter_Marriage::where('application_id', $report->id)->first()->help_type}}</td>
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Daughter_Marriage::where('application_id', $report->id)->first()->amount}}</td>

                  @elseif(App\Models\Back\Meritocracy::where('application_id', $report->id)->first())
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Meritocracy::where('application_id', $report->id)->first()->help_type}}</td>
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Meritocracy::where('application_id', $report->id)->first()->amount}}</td>

                  @elseif(App\Models\Back\Deadbody::where('application_id', $report->id)->first())
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Deadbody::where('application_id', $report->id)->first()->help_type}}</td>
                  <td class="align-middle text-center text-sm">{{App\Models\Back\Deadbody::where('application_id', $report->id)->first()->amount}}</td>
                  @endif
                  ...
                  <td class="align-middle text-center text-sm text-center">
                    ...
                    <a href="{{route('viewReport.boardsApplication',$report->id)}}" class="ml-2"><i class="fa-sharp fa-regular fa-eye"></i></a>
                    @endcan
                  </td>
                </tr>
                <?php $sl++; ?>
